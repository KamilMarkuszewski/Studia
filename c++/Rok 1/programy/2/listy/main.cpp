...
class utwor{
 public:
        utwor();                    //konstruktor domyslny
        utwor(int bitr);            //konstruktor z parametrem
        utwor(const utwor &);       //konstruktor kopiuj¹cy    
        ~utwor();                   //destruktor
     
        int  getbitrate() const;    //metody
        void setbitrate(int br);
        void setwsk_next(utwor *next) {wsk_next = next;}
        utwor *getwsk_next() const {return wsk_next;}
        void dodaj(utwor *&head);
  

 private:                        // dostepne jedynie wewnatrz dane
...
utwor::utwor(){                  //   konstruktor

               }

utwor::utwor(int bitr){          //   konstruktor z parametrem
    bitrate = bitr;

                       }

utwor::utwor(const utwor &cpy){  //   konstruktor kopiujacy
    wsk_next = new utwor;
    wsk_next = cpy.getwsk_next();

                               }


utwor::~utwor(){                //   destruktor nie robiacy nic

                }


//akcesory
int utwor::getbitrate() const {
    return bitrate;
      }

void utwor::setbitrate(int br){
    bitrate = br;
      }
...
void lista::ustaw_ost(){                                                       // utawia wskaznik na koniec
    ...
    utwor *wsk = (*wsk_aktualny).getwsk_next(); 
    for(;wsk != NULL ;){
        wsk_aktualny = wsk;
        wsk = (*wsk).getwsk_next();                                      
                                             }
