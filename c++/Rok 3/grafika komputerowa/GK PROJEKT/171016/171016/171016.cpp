...
void spinEgg(){
	...
	if(start2){
		...
		if( (kula_wspl[0][0] < -1.0) && b1 && (numerek < 6)){
			kula_wspl[0][0] += 0.5;
		} else {
			b2 = true;
		}

		if( (kula_wspl[1][0] < 6.0) && b2 && (numerek < 14)){
			kula_wspl[1][0] += 0.5;
		} else{
			if((kula_wspl[0][0] < -1.0) && (kula_wspl[1][0] < 6.0) ){
				b3 =true;
			}
		}

		if( (kula_wspl[2][0] < 10) && b3){
			kula_wspl[2][0] += 0.5;
		}
...
void keys(unsigned char key, int x, int y){
	...
	if(key == 'q') start2 = true;
	if(key == 'w') start2 = false;
}
